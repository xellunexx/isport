...
window.ICO = (function () {
  ...
  // Actual supplier drawings are delivered as product images.  Identify only near-white,
  // near-monochrome line art so alternate product renders are never mislabelled.
  function badgeTechnicalDrawings(host) {
    host.querySelectorAll("button[data-img] img").forEach(img => {
      const inspect = () => {
        try {
          ...
          let light = 0, chroma = 0, samples = 0;
          for (let i = 0; i < data.length; i += 4) { light += (data[i] + data[i + 1] + data[i + 2]) / 3; chroma += Math.max(data[i], data[i + 1], data[i + 2]) - Math.min(data[i], data[i + 1], data[i + 2]); samples++; }
          if (light / samples > 235 && chroma / samples < 9) {
            const button = img.closest("button"); button.classList.add("is-drawing"); button.setAttribute("aria-label", "Technical drawing");
          }
        } catch (_) { /* no badge if the browser cannot inspect an image */ }
      };
      if (img.complete) inspect(); else img.addEventListener("load", inspect, { once: true });
    });
  }
  ...
  function quickView(id) {
    ...
    badgeTechnicalDrawings($("#qvThumbs"));
    const chips = [];
