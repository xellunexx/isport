/* hover.js — the smart-hover engine.
   ...
   dwell 1150ms -> COMMIT:  product's cat+purpose+materials+brand APPLY as filters (auto-tagged,
                            clearly shown in the deck ribbon with the source product's thumb)
   Esc / wheel / leave cancels preview; committed filters stay until cleared (×, Esc, Clear all).
...
window.ICH = (function () {
  ...
  function lensShow(p, sig, phase) {
    ...
    lensEl.innerHTML = `
      ...
      <div class="lens-foot"><b>${n}</b> ${esc(T("results"))} · ${phase === "committed" ? "Esc — " + esc(T("reset")) : esc(T("lens_hint"))}</div>
  ...
  function arm(id, skipDelay) {
    ...
    if (previewId && previewId !== id) { // retarget fast
      ...
      resetTimers();
      tB = setTimeout(() => commit(id), COMMIT_MS - PREVIEW_MS);
      return;
    }
    tA = setTimeout(() => {
      ...
      tB = setTimeout(() => commit(armedId), COMMIT_MS - PREVIEW_MS);
